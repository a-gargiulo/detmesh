...
void mesh_diamond(int argc, char** argv, Diamond* diamond) {
  ...
  gmshModelGeoAddPoint(diamond->x1, tan(diamond->alpha * M_PI / 180.0) * diamond->x1, 0, lc, 2, &ierr);
  gmshModelGeoAddPoint(diamond->cx, diamond->cy, 0, lc, 3, &ierr);
  gmshModelGeoAddPoint(diamond->x2,tan(diamond->beta * M_PI / 180.0)*(diamond->l_d - diamond->x2), 0, lc, 4, &ierr);
