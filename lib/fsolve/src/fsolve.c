...
/*
  ...
  Parameters:
    ...
    Workspace, double WA1[N].

    Workspace, double WA2[N].
*/
{
  double alpha;
  double bnorm;
...
/*
  ...
  Parameters:
    ...
    Input, void FCN(int n,double x[], double fx[]): returns in fx[]
    the function value at the n-dimensional vector x[].
    ...
    Input, int ml, the number of subdiagonals within the band of the
    jacobian matrix. if the jacobian is not banded, set ml to at least n - 1.
...
/*
  ...
  Output:
    ...
    double fjac[n*n]: the approximate jacobian matrix at xn.
...
/*
  ...
  Parameters:
    ...
    Input, double epsfcn, determines a suitable step length for the 
    ...
    Input, int ldfjac, not less than n, specifies the leading 
...
/*
  Calculate the jacobian matrix.
*/
    fdjac_bdf2 ( dydt, n, t1, x1, t2, x2, t3, x3, fvec, fjac, ldfjac, ml, mu, 
      epsfcn, wa1, wa2 );
...
*/
    for ( j = 1; j <= n; j++ )
    {
      ...
      if ( wa1[j-1] == 0.0 )
      {
        printf ( "  hybrd_bdf2(): Matrix is singular.\n" );
      }
    }
/*
  Accumulate the orthogonal factor in FJAC.
...
*/
    for ( j = 1; j <= n; j++ )
    {
      ...
      if ( wa1[j-1] == 0.0 )
      {
        printf ( "  hybrd_be(): Matrix is singular.\n" );
      }
    }
/*
  Accumulate the orthogonal factor in FJAC.
...
*/
    for ( j = 1; j <= n; j++ )
    {
      ...
      if ( wa1[j-1] == 0.0 )
      {
        printf ( "  hybrd_tr: Matrix is singular.\n" );
      }
    }
/*
  Accumulate the orthogonal factor in FJAC.
...
/*
  ...
  Discussion:
    ...
    solution values there.  If yn satisfies the trapezoidal ODE solver
    condition, then
...
*/
{
  ...
  dydto = malloc ( n * sizeof ( double ) );
  dydtn = malloc ( n * sizeof ( double ) );
  ...
  for ( i = 0; i < n; i++ )
  {
    ft[i] = yn[i] - yo[i] - ( tn - to ) * 0.5 * ( dydto[i] + dydtn[i] );
  }
